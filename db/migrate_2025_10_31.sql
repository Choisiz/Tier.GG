...
-- 2) match_details 컬럼 마이그레이션
-- 2-1) game_result 컬럼 추가 (없으면)
ALTER TABLE match_details ADD COLUMN IF NOT EXISTS game_result BOOLEAN;
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_name = 'match_details' AND column_name = 'result'
  ) THEN
    EXECUTE 'UPDATE match_details SET game_result = result WHERE game_result IS NULL';
  END IF;
END $$;
