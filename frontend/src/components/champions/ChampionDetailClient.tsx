/* 챔피언 상세 페이지 클라이언트 렌더러 */
'use client';

import { useMemo, useState } from "react";
import Image from "next/image";
import Link from "next/link";
import { getDdragonImageByFile } from "@/lib/champions";
import type {
  ChampionDetailData,
...
interface Props extends ChampionInfoPageData {
...
type ActiveSpell = ChampionDetailData["spells"][number];
type PassiveSkill = ChampionDetailData["passive"];
...
function SkillIcon({
  ...
  const imageUrl = getDdragonImageByFile(
    version,
    isPassive ? "passive" : "spell",
    skill.image.full
  );
...
export default function ChampionDetailClient({
  ...
  const championImageUrl = getDdragonImageByFile(
    version,
    "champion",
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex">
                          ...
                          <span className="text-green-600 dark:text-green-400 font-medium">
                            {championData.stats.hpregen} (+{" "}
                            {championData.stats.hpregenperlevel})
                          </span>
                        </div>
                        <div className="flex">
                          <span className="text-gray-500 dark:text-gray-400">
                          ...
                          <span className="text-blue-600 dark:text-blue-400 font-medium">
                            ...
                            {championData.stats.mpregenperlevel})
                          </span>
                        </div>
                      </div>
                    </div>
